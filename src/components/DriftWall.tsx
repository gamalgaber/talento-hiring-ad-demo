...
const DriftWall = ({
  ...
  useEffect(() => {
    // Deliberately not a lazy useState initializer: that would run during
    // the initial client render and read window.matchMedia there, while SSR
    // rendered `false` — a hydration mismatch. Starting at `false` and
    // correcting it here (post-hydration) is the standard-safe pattern for
    // browser-only values.
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className={innerClass}>
        {/* Raw <img>, not next/image: this is a generic reusable component
            whose tile list gets cloned many times per column (see `copies`
            above) to fill the scroll loop — each clone reuses the same
            src, so there's no unique-image optimization to gain, and
            next/image's per-instance wrapper overhead would multiply by
            every clone for nothing. Native lazy-loading covers perf here. */}
